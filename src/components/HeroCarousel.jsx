...
function HeroCarousel() {
    ...
    async function fetchCarouselMovies() {
      try {
        ...
        if (data.results && Array.isArray(data.results)) {
          // Fetch more movies to ensure the dynamic list works well
  ...
  if (movies.length < 5) {
    return null; // Don't render if there aren't enough movies for the layout
  }

  const featuredMovies = movies.slice(0, 5);
  
  // Dynamically calculate the "Up Next" list based on the current slide
      ...
      <div className="row">
        <div className="col-lg-8">
          <Carousel
            ...
            showArrows={true}
            // This handler updates the component when the slide changes
          ...
          >
            {featuredMovies.map((movie) => (
              <div key={movie.id} className="hero-slide-container" onClick={() => navigate(`/details/movie/${movie.id}`)}>
                ...
                {/* This is the small poster image */}
                <img
